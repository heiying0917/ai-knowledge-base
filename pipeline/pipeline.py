...
def _generate_id(source: str, identifier: str) -> str:
    """生成知识条目 ID。

    格式: {source}-{YYYYMMDD}-{slug}

    Args:
        source: 来源缩写，如 github、rss。
        identifier: 唯一标识字符串，用于生成 slug。
    ...
    return f"{source}-{date_part}-{slug}"
...

{
  ...
  "summary": "中文摘要，100-300字，描述技术要点和价值",
  "tags": ["tag1", "tag2", "tag3"],
...
def step_analyze(raw_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for i, item in enumerate(raw_items):
        ...
        entry_id = _generate_id(
            "github" if source == "github_trending" else "rss",
            title or url,
        )
        ...
        if analysis_result and "score" in analysis_result:
            entry["analysis"] = {"score": analysis_result["score"]}
...
def step_save(items: list[dict[str, Any]], dry_run: bool = False) -> list[Path]:
    ...
    每条知识条目保存为独立 JSON 文件，文件名格式为 {id}.json。
    ...
    now_ts = _now_iso()
    for item in items:
        item["published_at"] = now_ts

        item_id = item.get("id", "unknown")
        filepath = ARTICLES_DIR / f"{item_id}.json"
        ...
        try:
            ...
            logger.info("已保存: %s", filepath.name)
        except OSError as exc:
            logger.error("写入失败: %s | %s", filepath.name, exc)

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M")
    raw_path = RAW_DIR / f"pipeline-{timestamp}.json"
    try:
        raw_path.write_text(
    ...
    logger.info("保存完成: %d 条文章 → %s", len(saved_paths), ARTICLES_DIR)
    return saved_paths
